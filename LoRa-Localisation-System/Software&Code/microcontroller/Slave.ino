...
void LoraSlaveMasterInitialization()
{
  ...
  Serial.println("TT: "+String(thisNodeAddr));
  thisNodeAddr|=((0x20&PINC)>>5);

  Serial.println("TT: "+String((0x20&PINC)>>1));
  
  thisNodeAddr|=((0x10&PINC)>>3);
  thisNodeAddr|=((0x08&PINC)>>1);
  thisNodeAddr|=((0x04&PINC)<<1);
  
  thisNodeAddr|=((0x02&PINC)<<3);
  thisNodeAddr|=((0x01&PINC)<<5);
  //thisNodeAddr|=0x3F&PINC;
  thisNodeAddr=~thisNodeAddr;
  if(thisNodeAddr==1)
  {
    nodeType = targetNode;
    nodeOrder = 0;
